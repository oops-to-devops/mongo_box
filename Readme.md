...
# https://gilt.readthedocs.io/en/latest/
  - git: https://github.com/oops-to-devops/mongo_box.git
    version: master
    dst: deployment/provisioners/mongo_box/
    post_commands:
      - make
```
...
```
      ...
      config.vm.provision "ansible" do |ansible|
          ansible.playbook = "provisioners/mongo_box/box_bootstrap_only.yml"
          ansible.galaxy_role_file = "provisioners/mongo_box/requirements.yml"
          ansible.galaxy_roles_path = "deployment/provisioners/mongo_box/roles"
          ansible.verbose = true
          ansible.groups = {
              "mongo_box" => [vconfig['vagrant_machine_name']]
          }
          ansible.extra_vars = {
...
resource "null_resource" "mongoboxed_servers" {
  ...
  provisioner "local-exec" {
    command = "$INFRASTRUCTURE_ROOT_DIR/provisioners/mongo_box/provision_box.sh"
